...
function Card({ className, children }: { className?: string; children: React.ReactNode }) {
  return (
    <motion.div
      variants={item}
      className={cn(
        "group relative flex min-w-0 flex-col overflow-hidden rounded-xl border border-aurora-border bg-aurora-bg-raised p-3 transition-colors duration-200 hover:border-aurora-border-strong sm:p-3.5",
        className,
      )}
    >
      {children}
...
export function HomePageClient({ stats, formatted }: Props) {
  return (
    <motion.section
      ...
      initial="hidden"
      animate="show"
      className="mx-auto grid w-full max-w-[1500px] gap-2 px-3 py-2 sm:gap-3 sm:px-6 sm:py-3 lg:max-h-[calc(100dvh-11rem)] lg:min-h-0 lg:flex-1 lg:grid-cols-12 lg:grid-rows-6 lg:gap-3 lg:overflow-hidden lg:px-8"
    >
      <Card className="lg:col-span-3 lg:row-span-3 lg:justify-between">
        <div>
          <span className="inline-flex w-fit items-center gap-2 rounded-full border border-aurora-border bg-aurora-panel px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.14em] text-aurora-fg-muted">
        ...
        <div className="flex items-stretch gap-2 pt-3">
          <Link href="/estimate" className="min-w-0 flex-1">
            <Btn size="sm" className="w-full justify-center px-3">
              Start estimate
            </Btn>
          </Link>
          <Link href="/methodology" className="min-w-0 flex-1">
            <Btn variant="secondary" size="sm" className="w-full justify-center px-3">
              Methodology
            </Btn>
          </Link>
        </div>
      ...
      <Card className="relative items-center justify-center overflow-hidden p-0 lg:col-span-6 lg:row-span-6 lg:min-h-0">
        <Orb className="absolute left-1/2 top-1/2 hidden h-[min(72%,22rem)] w-[min(72%,22rem)] -translate-x-1/2 -translate-y-[54%] lg:block" />
        <div className="relative z-10 flex h-full w-full min-h-0 flex-col gap-2 p-3 lg:justify-between">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-0.5 font-display text-xl font-semibold text-aurora-fg sm:text-2xl xl:text-3xl">
                {formatted.totalMgCO2e}
              </p>
              <p className="text-[11px] text-aurora-fg-muted">gCO₂e measured to date</p>
            </div>
            <div className="rounded-lg border border-aurora-border bg-aurora-panel px-2.5 py-1.5 text-right">
              <p className="font-mono text-[10px] uppercase tracking-[0.16em] text-aurora-fg-muted">Retired</p>
              <p className="font-display text-lg font-semibold text-aurora-carbon">{formatted.totalTonnesRetired}</p>
            </div>
          </div>
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-stretch">
              {steps.map((s, i) => (
                <div key={s.n} className="flex min-w-0 flex-1 items-center gap-1.5">
                  ...
                  {i < steps.length - 1 && (
                    <span aria-hidden="true" className="hidden shrink-0 text-aurora-fg-muted sm:block">
                      ›
                    </span>
                  )}
                </div>
        ...
        <div className="mt-4 border-t border-aurora-border/60 pt-3 lg:mt-auto">
          ...
          <div className="mt-2.5 space-y-2">
            {[
              ...
              <div key={f.k}>
                <span className="text-[11px] text-aurora-fg-muted">{f.k}</span>
                <p className="font-display text-base font-semibold text-aurora-fg">{f.v}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <ul className="space-y-1.5 text-[11px]">
          {activity.map((a) => (
            <li key={a.label} className="flex items-center justify-between gap-2">
              <span className="flex min-w-0 items-center gap-1.5 text-aurora-fg-muted">
              ...
              <span className="w-7 shrink-0 text-right font-mono text-aurora-fg-muted">{a.pct}%</span>
            </li>
          ))}
        </ul>
      </Card>

      <Card className="justify-center py-2 lg:col-span-3 lg:col-start-1 lg:row-span-1 lg:row-start-6">
        <Link href="/certificate/demo" className="flex items-center justify-between gap-3">
          <span className="flex min-w-0 items-center gap-2.5">
            <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-aurora-border bg-aurora-panel text-aurora-carbon">
              <VerifiedIcon className="h-4 w-4" />
            </span>
            <span className="min-w-0">
              <span className="block text-xs font-medium text-aurora-fg">Carbonmark verified</span>
              <span className="block truncate text-[11px] text-aurora-fg-muted">on Avalanche</span>
            </span>
          </span>
      ...
      <Card className="lg:col-span-3 lg:col-start-10 lg:row-span-3 lg:row-start-4 lg:justify-between">
        <div>
          ...
          <p className="mt-2 text-[11px] leading-relaxed text-aurora-fg-muted">
          ...
          <ul className="mt-2 space-y-1.5 text-[11px]">
            {[
              ...
              <li
                key={r.k}
                className="flex items-center justify-between gap-2 border-b border-aurora-border/70 pb-1.5 last:border-0"
              >
                <span className="text-aurora-fg-muted">{r.k}</span>
                <span className="truncate font-mono text-[10px] text-aurora-fg">{r.v}</span>
              </li>
            ))}
          </ul>
        </div>
        <div className="mt-2 grid grid-cols-3 gap-1.5 lg:mt-0">
          {[
            ...
            <div key={s.l} className="rounded-lg border border-aurora-border bg-aurora-panel p-2 text-center">
              <p className="font-display text-sm font-semibold text-aurora-fg">{s.v}</p>
              <p className="mt-0.5 text-[9px] text-aurora-fg-muted">{s.l}</p>
            </div>
          ))}
        </div>
      </Card>
